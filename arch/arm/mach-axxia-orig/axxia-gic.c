...
static void axxia_ipi_demux(struct pt_regs *regs)
{
	...
	do {
		...
		if (all & (1 << MUX_MSG_WORK))
			handle_IPI(6, regs); /* 0 = ARM IPI_IRQ_WORK */
		if (all & (1 << MUX_MSG_COMPLETE))
			handle_IPI(7, regs); /* 0 = ARM IPI_COMPLETION */
...
void axxia_gic_raise_softirq(const struct cpumask *mask, unsigned int irq)
{
	...
	case 6: /* IPI_IRQ_WORK */
		regoffset += 0x8; /* Axxia IPI2 */
		muxed_ipi_message_pass(mask, MUX_MSG_WORK);
		break;

	case 7: /* IPI_COMPLETION */
		regoffset += 0x8; /* Axxia IPI2 */
		muxed_ipi_message_pass(mask, MUX_MSG_COMPLETE);
		break;
